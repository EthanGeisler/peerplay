...
- **Game launch is direct** — `installedStore.launch()` spawns the exe immediately. No license verification or DRM checks at launch time. Linux game launching is not yet supported (Windows .exe games won't run natively).
- **Linux build targets:** AppImage + deb. Config in `client/package.json` `"linux"` section. Icon must be PNG (`resources/icon.png`), not ICO. `homepage` field required for deb.
